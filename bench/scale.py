#!/usr/bin/env python3
"""Scale pilot: does the physical prompt stay bounded as the corpus grows?

DCY's central claim is that addressable corpus size (VT, measured with a fixed
tokenizer) can be decoupled from physical prompt size (PT). This measures that
directly: the same tasks are run against indexes of increasing VT at a fixed
byte budget, and prompt tokens are counted with a real tokenizer rather than
estimated from bytes.

The quantity of interest is d(prompt_tokens)/d(VT). If DCY works, it is ~0 and
recall degrades gracefully; if the prompt grows with the corpus, the claim
fails regardless of recall. VCR = VT / PT is reported as an addressability
ratio only — it says nothing about comprehension.
"""
...
def main():
    ...
    for pair in args.scales:
        ...
        # Tasks whose gold is absent from this smaller index cannot be scored here.
        for budget in args.budgets:
            recalls, prompt_tokens, latencies, scored = [], [], [], 0
            for task in tasks:
                if not set(task["gold"]) <= set(names.values()):
                    continue
                scored += 1
                rendered, elapsed = view(args.dcy, db, args.mode, task["goal"], budget)
            ...
            if not scored:
                continue
            records.append({
                "vt_tokens": vt, "entities": entity_count, "budget_bytes": budget,
                "tasks_scored": scored,
                ...
                "latency_ms_p95": round(sorted(latencies)[max(0, int(0.95 * len(latencies)) - 1)], 2),
    ...
    report = {
        "kind": "scale_pilot", "encoding": args.encoding, "mode": args.mode,
        "split": args.split, "tasks_in_split": len(tasks),
        "records": records, "prompt_growth": slopes,
        "limitations": [
            "VT is corpus size under a fixed tokenizer, not model comprehension",
            "VCR is an addressability ratio and is trivially inflatable by adding junk",
            "injected recall is not task success",
            "tasks whose gold is absent from a smaller index are skipped at that scale",
        ],
    }
    text = json.dumps(report, indent=2)
    if args.out:
        args.out.write_text(text + "\n")
    print(f"{'VT':>10s} {'budget':>7s} {'n':>4s} {'recall':>7s} {'prompt_tok':>11s} "
          f"{'p95':>7s} {'VCR':>8s} {'p95_ms':>7s}")
    for r in records:
        print(f"{r['vt_tokens']:10,} {r['budget_bytes']:7d} {r['tasks_scored']:4d} "
              f"{r['injected_recall']:7.3f} {r['prompt_tokens_mean']:11.1f} "
              f"{r['prompt_tokens_p95']:7.0f} {r['vcr']:8.1f} {r['latency_ms_p95']:7.1f}")
