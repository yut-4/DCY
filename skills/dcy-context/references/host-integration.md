...
5. When exact code is needed, the host calls `dcy_source({"entity_id":42,"generation":"...","max_bytes":1024})` and feeds only the needed bounded span. This MVP cannot page an oversized entity. Re-run `index` after edits and create a new goal if generation changed.
